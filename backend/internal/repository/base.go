...
func dbForContext(ctx context.Context, fallback *gorm.DB) *gorm.DB {
	if tx, ok := ctx.Value(transactionDBKey{}).(*gorm.DB); ok && tx != nil {
...
func withTransaction(ctx context.Context, db *gorm.DB, fn func(context.Context) error) error {
	return db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		return fn(context.WithValue(ctx, transactionDBKey{}, tx))
	})
}
...
func (s *Store[T]) List(ctx context.Context, query dto.PageQuery) (Page[T], error) {
	...
	db := dbForContext(ctx, s.db).Model(new(T))
...
func (s *Store[T]) Delete(ctx context.Context, id uint) error {
	result := dbForContext(ctx, s.db).Delete(new(T), id)
	if result.Error != nil {
		return result.Error
	}
	if result.RowsAffected == 0 {
...
func (s *Store[T]) CountByStatus(ctx context.Context) (map[string]int64, error) {
	rows, err := dbForContext(ctx, s.db).Model(new(T)).
